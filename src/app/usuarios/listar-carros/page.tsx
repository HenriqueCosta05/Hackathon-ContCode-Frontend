...
export default function ListarCarros() {
  ...
  return (
    <>
      <h1 className="text-4xl font-bold py-6 text-center">Carros</h1>
      <main className="flex justify-center items-center flex-wrap gap-4 w-full h-screen">
        {carrosMock?.slice(0, visibleCards).map((carro: Partial<Carro>) => (
          <CardComponent
            ...
            action={{
              text: "Ver mais",
              href: "/usuarios/consultar-manutencoes",
            }}
            bullets={[
              { key: "Ano", value: carro.year?.toString() || "" },
              { key: "Marca", value: carro.brand || "" },
            ]}
          />
        ))}
